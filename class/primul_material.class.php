...
class PrimulMaterial {
	private $connection;
	public function __construct($connection=null, $iddestinatie="") {
		$this->destinatie=$iddestinatie;
		$stmt = $connection->prepare("SELECT p.id, p.nume FROM primul_material p WHERE p.id<800");
		$stmt->execute();
		$stmt->store_result();
	...
	public function get_primulMaterial(){
	      return $this->results;
	}
	public function get_primulMaterial_html()
	{
